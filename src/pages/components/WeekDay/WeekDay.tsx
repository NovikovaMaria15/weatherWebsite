...
class WeekDay extends Component<any, any> {
  constructor(props: any) {
    super(props);
    this.state = { dayDayDay: '', open: true };
    this.handleClick = this.handleClick.bind(this);
  }

  handleClick = (dt: any) => {
    const dayDayDay = this.props.weekDaily.find((week: any) => {
      if (week.dt === dt) {
        return true;
      }
      return false;
    });
    this.setState({ dayDayDay, open: false });
  };

  render() {
    const { weekDaily } = this.props;
    const { open } = this.state;
    const { dayDayDay } = this.state;
    ...
    return (
      <>
        <S.AlignmentIconsWeek>
          {weekDaily.map((weekDail: any) => (
            ...
            <div
              className="click"
              key={weekDail.dt}
              onClick={() => this.handleClick(weekDail.dt)}
            >
              <Weather
        ...
        {dayDayDay && (
          <Current
            dayCurrent={new Date(dayDayDay.dt * 1000).toLocaleString(
              'default',
              {
                month: 'long',
                day: 'numeric',
        ...
        {dayDayDay && (
          <WeatherItems
            ...
            sunset={new Date(dayDayDay.sunset * 1000).toLocaleString(
              'default',
              {
                hour: '2-digit',
                minute: '2-digit',
...
const mapStateToProps = (state: any) => ({
  weekDaily: state.weekWeather.daily,
  dayDayDay: state.dayDayDay,
});
